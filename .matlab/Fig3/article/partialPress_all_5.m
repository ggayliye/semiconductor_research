...
% FIXED PARAMETERS
%% =====================================================
F_frac_init = 1e-15;          % very small fraction
GR_vec = [0.01, 0.3125 : 0.3125 : 100];
N = length(GR_vec);
...
%% =====================================================
x0 = log([6e-4; 1.98e-13]);   % b0 and c0

%% =====================================================
% SCALING FACTORS FOR LARGE COEFFICIENTS
%% =====================================================
scale_a = 1;    
scale_d = 1e-9;
scale_e = 1e-18;
...
for i = 1:N
    ...
    try
        [xsol,~,~,ef] = lsqnonlin(@(x) eqns_bc_safe_dynamic(x,F_frac,GR,scale_a,scale_d,scale_e,scale_f,scale_h,scale_g), ...
                                  x0, [], [], options);
        exitflag_vec(i) = ef;
    catch ME
    ...
    a = 143355396545689*(b^2)*(c^(3/2)) * scale_a;
    d = 50567580842.629*(b^2)*(c^(3/4)) * scale_d;
    e = 7.01432630423488E+21*(b^3)*(c^(3/2)) * scale_e;
    f = 1.07688142574839E+40*(b^6)*(c^3) * scale_f;
    ivar = 7.53238785702308E-08*((3/2)*(b + 50567580842.629*(b^2)*(c^(3/4)) + 7.01432630423488E+21*(b^3)*(c^(3/2))) ...
           - 2*(c - 1.86489627968959E-20/c_safe) + 3*(1.07688142574839E+40*(b^6)*(c^3)) ...
           - ((3/2)*(0.0005 - (4/3)*GR*0.0005/2))) / sqrt(c_safe + 1e-50); 
    ivar_for_sqrt = max(ivar,0) + 1e-40;  
    h = 162965559517.107*b*(c^(3/4)) * sqrt(ivar_for_sqrt) * scale_h;
    jvar = (3/2)*(b + 50567580842.629*(b^2)*(c^(3/4)) + 7.01432630423488E+21*(b^3)*(c^(3/2))) ...
           - 2*(c - 1.86489627968959E-20/c_safe) + 3*(1.07688142574839E+40*(b^6)*(c^3)) ...
           - (3/2)*(0.0005 - (4/3)*GR*0.0005/2);
    g = 1.86489627968959E-20 / c_safe * scale_g;
    P_IG = ((1 - F_frac)/F_frac) * (0.5*h + ivar + jvar);

    %% ---- Check for negative and adjust F_frac if necessary ----
    ...
    %% ---- Continuation ----
    x0 = xsol;  % use previous solution as next initial guess
end
...
%% =====================================================
function F = eqns_bc_safe_dynamic(x,F_frac,GR,scale_a,scale_d,scale_e,scale_f,scale_h,scale_g)

b = exp(x(1));
c = exp(x(2));
c_safe = max(c,1e-50);

% ---- Derived variables (new equations) ----
a = 143355396545689*(b^2)*(c^(3/2)) * scale_a;
d = 50567580842.629*(b^2)*(c^(3/4)) * scale_d;
e = 7.01432630423488E+21*(b^3)*(c^(3/2)) * scale_e;
f = 1.07688142574839E+40*(b^6)*(c^3) * scale_f;
